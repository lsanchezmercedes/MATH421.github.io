  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(
        ...
        choices = unique(d$Name), selected="ZTS"),
      ...
      sliderInput(inputId = "close",
                  "Select Stock Price close range:",
                  min = min(d$close, na.rm=TRUE),
                  max = max(d$close, na.rm=TRUE),
                  value= c(14, 2000)),
    ),
...
server <- function(input, output) {
  ...
  output$show_plot <- renderPlot({
    ...
    d <- d %>% filter(Name %in% company, date>input$date[1],  date<input$date[2],
                      Increase %in% input$increase, close>=input$close[1], close<=input$close[2]) 
    
    r <- d %>% ggplot(aes(x = d[["volume"]]))+
      geom_density() +
      labs(x="Volume")
